...
class MaitriPortalService:
    ...
    async def _resolve_station_id(self) -> UUID | None:
        ...
        if MaitriPortalService._STATION_ID_CACHE is not None:
            return MaitriPortalService._STATION_ID_CACHE
        station = await self.station_repo.get_by_code(self.STATION_CODE)
        MaitriPortalService._STATION_ID_CACHE = station.id if station else None
        return MaitriPortalService._STATION_ID_CACHE
